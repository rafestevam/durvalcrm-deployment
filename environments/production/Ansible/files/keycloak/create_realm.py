...
def get_admin_token(session):
    ...
    try:
        ...
        response = session.post(token_url, data=data, verify=False, allow_redirects=True)
        print(f"Token response status: {response.status_code}")
        if response.history:
            print(f"Redirects occurred: {[r.url for r in response.history]}")
        response.raise_for_status()
        return response.json()['access_token']
    except Exception as e:
        print(f"Error getting admin token: {e}")
        print(f"Response URL: {response.url if 'response' in locals() else 'N/A'}")
